...
The project can be packaged as a single executable using PyInstaller.
...
The output binary is not a native binary in the Go/Rust sense. It is a bundled Python application that includes the interpreter and dependencies.

That is usually good enough for operational distribution, because the end user does not need to install:
...
### `make example` fails
This usually means at least one example check failed, which may be intentional.

Inspect the output and the example YAML file.

### `make test` fails with `No module named pytest`
Install `pytest` in the virtual environment or make sure the Makefile installs it.

### Binary runs locally but not on another machine
PyInstaller builds are platform-specific and may depend on the target environment.

Build on the same type of system you plan to run on.

### JSON assertion fails even though output looks correct
Check:

- did you use `--parser json`?
- is stdout valid JSON?
- is the JSON path correct?
- is the expected value exactly what the code compares against?

### `stdout-equals` unexpectedly fails
Remember that exact equality is strict. Trailing newlines and spaces matter.

Use `contains` or `regex` if exact matching is too strict.

---

## Design notes

This project intentionally keeps a small scope.

Current design choices:

- `argparse`-based CLI for low dependency and easy comprehension
- YAML config for readable, declarative checks
- explicit internal models for clarity
- report rendering separated from execution
- simple exit-code semantics for CI compatibility

This makes it easy to return to the repository later and understand the flow:

1. CLI parses input
2. config loader builds `CheckSpec`
3. runner executes checks
4. assertions validate results
5. reporters render output
6. process exits with automation-friendly status
...
- the checks should be version-controlled
- other people should be able to read and edit them
- you want stable smoke checks in CI/CD

### Use inline CLI checks when:
- you are experimenting
- you want one quick ad hoc verification
- you are debugging a command
- you are testing the tool itself

### Use JSON output when:
- another tool consumes the results
- you want structured automation
- you plan to archive outputs

### Use summary table when:
- a human is reading the output
- you want quick terminal readability
...
Some reasonable future improvements:

- stricter CLI validation for invalid argument combinations
- richer error messages when JSON flags are used incorrectly
- support for stderr assertions
- support for numeric comparisons
- support for multiple subcommands
- richer YAML schema validation
...
That is the whole project in practical terms.
